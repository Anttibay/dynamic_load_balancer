        ...
        data_schema = vol.Schema(
            {
                vol.Required(
                    CONF_FUSE_SIZE,
                    default=current_config.get(CONF_FUSE_SIZE, DEFAULT_FUSE_SIZE),
                ): NumberSelector(
                    NumberSelectorConfig(
                        min=10,
                        max=125,
                ...
                vol.Required(
                    CONF_AGGRESSIVENESS,
                    default=current_config.get(CONF_AGGRESSIVENESS, DEFAULT_AGGRESSIVENESS),
                ): SelectSelector(
                    SelectSelectorConfig(
                        options=[
                            {"value": "low", "label": "Low (95% capacity)"},
                ...
                vol.Required(
                    CONF_SPIKE_FILTER_TIME,
                    default=current_config.get(CONF_SPIKE_FILTER_TIME, DEFAULT_SPIKE_FILTER_TIME),
                ): NumberSelector(
                    NumberSelectorConfig(
                        min=5,
                        max=300,
